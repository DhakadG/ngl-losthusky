...
const HOSTING_RE =
  /\b(host(?:ing)?|vpn|proxy|datacenter|data ?center|colo(?:cation)?|cloud|digitalocean|amazon|aws|google ?cloud|microsoft ?azure|\bovh\b|linode|vultr|contabo|leaseweb|hetzner|choopa|m247|psychz|server(?:s)?\b)/i;
...
async function checkAuth() {
  if ((await api("/api/admin/me")).authed) showDash();
}
// password show/hide
...
async function loadStats() {
  ...
  $("stats").innerHTML = [
    ...
    .map(([l, n]) => `<div class="stat"><div class="n">${n || 0}</div><div class="l">${l}</div></div>`)
    .join("");
}
...
window.whoSent = async function (visitorId) {
  ...
  const hostingWarn = looksHosting(e.isp) ? `<p class="muted" style="color:#ff9db0;margin-top:6px">⚠️ ISP name matches hosting/VPN/proxy patterns — may not be a real residential visitor.</p>` : "";
...
window.shareStory = async function (id) {
  ...
  g.addColorStop(0, "#ff2e74");
  g.addColorStop(1, "#ff9a3c");
  roundRectPath(ctx, cardX, cardY, cardW, headH, radius, radius, 0, 0);
  ctx.fillStyle = g;
  ctx.fill();
  // body
  const bodyH = 560;
  ...
  ctx.fillText("send me anonymous messages!", W / 2, cardY + 175);
  ...
  a.download = "losthusky-story.png";
  a.click();
};
...
const SETTING_META = [
  ...
  [
    "skip_bot_views",
    "Hide bot / crawler views",
    "Skip view notifications from hosting / VPN / proxy networks (Amazon, Cloudflare, datacenters). These are almost always link-preview crawlers, not real people.",
  ],
  [
    "skip_bot_messages",
    "Hide bot / crawler messages",
    "Skip message notifications from hosting / VPN / proxy networks. Off by default — real messages are rare and worth seeing even from odd networks.",
  ],
];
async function loadSettings() {
  ...
  if (!r.ok) {
    el.innerHTML = `<p class="muted">Couldn't load settings.</p>`;
    return;
  }
  const s = r.settings;
  el.innerHTML =
...
async function loadSenders() {
  ...
  const el = $("sendersTab");
  if (!r.ok || !r.visitors.length) {
    el.innerHTML = `<p class="muted">No senders yet.</p>`;
    return;
  }
  el.innerHTML = r.visitors
    .map(
      (v) => `
    <div class="listrow">
...
$("newLinkBtn").addEventListener("click", () => {
  openModal(`<div class="modal-head"><h2>Create a link</h2><button class="x" onclick="closeModal()">×</button></div>
    <div class="mbody">
      ...
      <input id="nlPrompt" placeholder="prompt (send me anonymous messages!)" style="width:100%;padding:12px;border-radius:10px;border:1px solid var(--border);background:var(--bg);color:#fff;margin-bottom:12px" />
  ...
  $("nlCreate").addEventListener("click", async () => {
    const r = await api("/api/admin/links", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ slug: $("nlSlug").value.trim(), prompt: $("nlPrompt").value.trim() }),
    });
    if (r.ok) {
      closeModal();
      document.querySelector('.tab[data-tab="links"]').click();
    } else alert("Invalid slug (use letters, numbers, - or _)");
  });
});
